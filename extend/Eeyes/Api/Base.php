...
class Base
{
    ...
    {
        if (substr($url, 0, 1) === '/') {
            $eeyes_api_url = Env::get('eeyes.api_url');
            if (!$eeyes_api_url) {
                throw new \Exception('eeyes.api_url must be set.');
            }
    ...
    {
        $eeyes_api_token = Env::get('eeyes.api_token');
        if (!$eeyes_api_token) {
            throw new \Exception('eeyes.api_token should be set.');
        }
    ...
    public static function curl($url, $postfields = null)
    {
        ...
        if (substr($url, 0, 8) === 'https://' && Env::get('app_debug')) {
